...
通常は追加不要。
この repo のルートにある `PULL_REQUEST_TEMPLATE.md` は、各 repo に同名ファイルがない場合に default community health files として自動参照される。

各 repo で独自テンプレートに上書きしたい場合だけ、`.github/PULL_REQUEST_TEMPLATE.md` を配置する。
